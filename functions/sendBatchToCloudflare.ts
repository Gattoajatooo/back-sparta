// Função auxiliar reutilizável para enviar lotes para Cloudflare
...
function fillVariables(content, contact, user, company) {
    ...
    message = message.replace(/{{\s*current_date\s*}}/gi, currentDate);
    message = message.replace(/{{\s*current_time\s*}}/gi, currentTime);
    message = message.replace(/{{\s*current_day\s*}}/gi, currentDay);
    message = message.replace(/{{\s*current_month\s*}}/gi, currentMonth);
    message = message.replace(/{{\s*current_year\s*}}/gi, currentYear);
...
export async function sendBatchToCloudflare({
    ...
    try {
        console.log('\n📤 ========== ENVIANDO PARA CLOUDFLARE ==========');
        console.log(`📦 Batch ID: ${batch_id}`);
        console.log(`👥 Destinatários: ${recipients.length}`);
        console.log(`📱 Sessões: ${selected_sessions.length}`);
        ...
        console.log(`   ✓ ${activeSessions.length} sessões ativas encontradas`);

        // 3. Preparar distribuição de sessões
        let sessionIndex = 0;
        const getNextSession = () => {
            if (session_sending_strategy === 'random') {
                ...
                // Smart: escolher sessão com menos mensagens enviadas
        ...
        for (let i = 0; i < recipients.length; i++) {
            ...
            // Buscar contato completo para preencher variáveis
            let contact = null;
            try {
                contact = await base44.entities.Contact.get(recipient.contact_id);
            } catch (error) {
                console.warn(`   ⚠ Contato ${recipient.contact_id} não encontrado, usando dados básicos`);
                contact = recipient;
            }
            ...
            if (!chat_id) {
                console.warn(`   ⚠ Telefone inválido para ${recipient.name}, pulando...`);
                continue;
            }
        ...
        console.log(`\n   📝 Criando ${messagesToCreate.length} mensagens no banco...`);

        // 6. Criar mensagens em lote no banco
        const createdMessages = await base44.entities.Message.bulkCreate(messagesToCreate);
        console.log(`   ✓ ${createdMessages.length} mensagens criadas no banco`);

        // 7. Enviar jobs em lote para Cloudflare
        console.log(`\n   ☁️  Enviando ${cloudflareJobs.length} jobs para Cloudflare...`);
        ...
        console.log(`   ✓ ${cloudflareResult.created_count} jobs agendados no Cloudflare`);

        // 8. Atualizar mensagens com scheduler_job_id
        if (cloudflareResult.jobs && cloudflareResult.jobs.length > 0) {
            console.log(`\n   🔄 Atualizando mensagens com scheduler_job_id...`);
            ...
            console.log(`   ✓ Mensagens atualizadas com scheduler_job_id`);
        }
        ...
        console.error('\n❌ Erro ao enviar para Cloudflare:', error);
        return {
            success: false,
            error: error.message
        };
