...
Deno.serve(async (req) => {
    ...
    try {
        ...
        const { phone, file, sessionName, contactId, sessionProfile } = await req.json();
        ...
        const cleanBase = apiUrl.endsWith('/') ? apiUrl.slice(0, -1) : apiUrl;
        const sendFileUrl = `${cleanBase}/api/sendFile`;
        ...
        try {
                ...
                if (contactId) {
                    try {
                        ...
                        if (newTemp !== currentTemp) {
                            ...
                            console.log(`[sendFile] 🌡️ Temperatura atualizada: ${currentTemp} -> ${newTemp}`);
                        }
                    } catch (tempError) {
                        console.warn('[sendFile] ⚠️ Erro ao atualizar temperatura:', tempError.message);
                    }
                }
            ...
            const duration = Date.now() - startTime;
            console.error('Erro ao processar envio de arquivo:', formatError);
