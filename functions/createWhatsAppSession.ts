...
// Função para gerar hash único para nome da sessão
...
Deno.serve(async (req) => {
    try {
        ...
        // Usar sempre a empresa do usuário atual
        const targetCompanyId = user.company_id;

        // 1. VERIFICAR LIMITE DE SESSÕES DO PLANO
        ...
        if (!planId) {
             return Response.json({ error: 'Não foi possível identificar seu plano atual.' }, { status: 403 });
        }

        const currentPlan = await base44.asServiceRole.entities.Plan.get(planId);

        if (!currentPlan) {
            return Response.json({ error: 'Plano da assinatura ativa não encontrado.' }, { status: 403 });
        }
        ...
        if (sessionLimit !== -1) {
            ...
            if (activeSessions.length >= sessionLimit) {
                return Response.json({ 
                    error: `Limite de ${sessionLimit} sessões ativas atingido para o seu plano (${currentPlan.name}). Faça upgrade ou delete uma sessão.` 
                }, { status: 403 });
            }
        }
        ...
        if (!apiKey || !apiUrl || apiKey.trim().length === 0) {
            return Response.json({ 
                error: 'Configurações WAHA não encontradas ou inválidas.' 
            }, { status: 500 });
        }
        ...
        console.log(`[createWhatsAppSession] 📤 POST ${fullUrl}`);
        ...
        try {
            ...
            console.error('[createWhatsAppSession] ❌ Parse error');
            
            return Response.json({
                error: 'Resposta inválida da API WAHA',
        ...
        if (!wahaResponse.ok) {
            console.error(`[createWhatsAppSession] ❌ WAHA error: ${wahaResponse.status}`);
            
            if (wahaResponse.status === 401) {
                return Response.json({
                    error: 'ERRO DE AUTENTICAÇÃO: API Key inválida',
                    message: 'Verifique se a WAHA_API_KEY está correta'
                }, { status: 401 });
            }
            
            return Response.json({
                error: 'Erro ao criar sessão no WAHA',
                status: wahaResponse.status,
                details: wahaData
            }, { status: wahaResponse.status });
        }
        ...
        const savedSession = await base44.asServiceRole.entities.Session.create({
        ...
        console.log(`[createWhatsAppSession] ✅ Sessão criada: ${sessionName}`);
        ...
        console.error('[createWhatsAppSession] ❌ Erro:', error.message);
